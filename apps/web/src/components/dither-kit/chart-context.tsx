...
export function useChartController({
  ...
  /** Pin the y-scale domain (skips auto/`nice`). */
  ...
  const yDomainLo = yDomain?.[0]
  const yDomainHi = yDomain?.[1]
  const y = useMemo(() => {
    const pinned =
      yDomainLo != null && yDomainHi != null
        ? ([yDomainLo, yDomainHi] as [number, number])
        : undefined
    return buildYScale(min, max, plotHeight, pinned)
  }, [min, max, plotHeight, yDomainLo, yDomainHi])
